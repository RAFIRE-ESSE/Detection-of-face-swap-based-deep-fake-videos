...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            nn.ConvTranspose1d( nz, ngf * 8, 8, 1, 0, bias=False),
            nn.BatchNorm2d(ngf * 8),
            nn.ReLU(True),
            nn.ConvTranspose1d(ngf * 8, ngf * 4, 6, 2, 1, bias=False),
            nn.BatchNorm2d(ngf * 4),
            nn.ReLU(True),
            nn.ConvTranspose1d( ngf * 4, ngf * 2, 2, 2, 1, bias=False),
            nn.BatchNorm2d(ngf * 2),
            nn.ReLU(True),
            nn.ConvTranspose1d( ngf * 2, ngf, 2, 2, 1, bias=False),
            nn.BatchNorm2d(ngf),
            nn.ReLU(True),
            nn.ConvTranspose1d( ngf, nc, 7, 3, 1, bias=False),
            nn.Tanh()
        )
